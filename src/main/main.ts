...
let gameplayQuickOverlayTransitionQueue: Promise<void> = Promise.resolve();
...
let gameplayQuickOverlayPreparedWindowHandle: number | null = null;
...
let loadingHomePromotion: Promise<void> | null = null;
let loadingHomeNextProbeAt = 0;
...
function protectGameplayQuickOverlayDuringResume(): void {
  ...
  const overlay = getLiveGameplayQuickOverlayWindow();
  if (!overlay) return;
  // Keep the already-painted surface topmost until the exact tracked game is
  // visible and foreground. Lowering or blurring it here exposes whichever
  // unrelated window happens to be next in the desktop z-order.
...
async function enforceGameplayQuickOverlayZOrder(
  overlay: BrowserWindow,
  minimizeGameAfterPaint = false
): Promise<boolean> {
  const overlayHandle = browserWindowNativeHandle(overlay);
  const state = await enforceQuickOverlayZOrder(
    overlayHandle,
    gameplayQuickOverlayPreparedWindowHandle ?? undefined,
  ...
  const verified = Boolean(
    ...
    (state.overlayForeground || (minimizeGameAfterPaint && !state.gameVisible))
  );
  gameplayQuickOverlayNativeZOrderVerified = verified;
  await logLine(
    verified ? 'info' : 'warn',
    `Quick overlay native z-order ${verified ? 'verified' : 'failed'}: ` +
      `overlay=${overlayHandle}, game=${gameplayQuickOverlayPreparedWindowHandle ?? 0}, ` +
      ...
      `captured-frame fallback=${minimizeGameAfterPaint}.`
      + (state?.error ? ` Native worker: ${state.error}` : '')
  );
  return verified;
}

async function replaceUnsafeDirectBackdrop(overlay: BrowserWindow, gameId: string | null, allowLoadingHome = false): Promise<void> {
  if (
  ...
  await logLine(
    'warn',
    'Direct game staging was not verified; keeping the game visible while NXGS paints its exact tracked frame.'
  );
  if (allowLoadingHome) await prepareGameplayQuickOverlayRenderer(true, false, true, true);
  else await prepareGameplayQuickOverlayRenderer(true, false, true);
}

async function stageGameplayQuickOverlay(
  overlay: BrowserWindow,
  gameId: string | null,
  ...
  const exactCapturedFrameReady =
    ...
    ['live', 'snapshot'].includes(gameplayQuickOverlayPreparedBackdropKind ?? '') &&
    Number(gameplayQuickOverlayPreparedWindowHandle) > 0;
  if (!exactCapturedFrameReady) {
    await logLine(
      'warn',
      'Native live overlay staging failed and no exact captured game frame was ready; ' +
        'NXGS left the game visible instead of exposing another window.'
    );
    return false;
  }
  ...
  // focus from its owning process without switching through another app.
  overlay.focus();
...
async function performGameplayQuickOverlayPreparation(
  ...
  const preparationGeneration = gameplayQuickOverlayTransitionGeneration;
  ...
  const loadingHomeBlocksPreparation = () => allowLoadingHome
    ? gameplayLaunchCoverGameId !== gameId || !gameplayQuickOverlayDesiredOpen ||
      preparationGeneration !== gameplayQuickOverlayTransitionGeneration
  ...
  if (loadingHomeBlocksPreparation()) return;
  if (allowLoadingHome && !['direct', 'live'].includes(backdrop.kind)) return;
  if (launcher.activeState.game?.id !== gameId || overlay.isDestroyed()) return;
  ...
  gameplayQuickOverlayPreparedWindowHandle = backdrop.capturedWindowHandle ?? null;
  gameplayQuickOverlayRendererReady = painted;
  if (launcher.activeState.status !== 'quickOverlayOpen' && gameplayLaunchCoverGameId !== gameId) {
    overlay.hide();
  }
  await logLine(
...
async function prepareGameplayQuickOverlayRenderer(
  ...
  const preparedBackdropIsAcceptable = (): boolean =>
    ...
    gameplayQuickOverlayPreparedBackdropKind === 'snapshot' ||
  ...
  const operation = performGameplayQuickOverlayPreparation(gameId, captureSnapshot, preferDirectGameplay, allowLoadingHome);
...
async function performGameplayQuickOverlayShow(): Promise<void> {
  ...
  if (overlay.isVisible() && gameplayQuickOverlayPreparedGameId === gameId) {
    ...
    overlay.focus();
    overlay.webContents.focus();
    await stageGameplayQuickOverlay(overlay, gameId);
    if (!gameplayQuickOverlayDesiredOpen || overlay.isDestroyed()) return;
    overlay.webContents.focus();
    return;
  }
  ...
  const exactGameVisualPrepared =
    gameplayQuickOverlayRendererReady &&
    ['direct', 'live', 'snapshot'].includes(gameplayQuickOverlayPreparedBackdropKind ?? '') &&
    Number(gameplayQuickOverlayPreparedWindowHandle) > 0;
  if (!exactGameVisualPrepared) {
    await logLine(
      'warn',
      'Home overlay remained hidden because no exact tracked game window or frame was ready; ' +
        'the running game was left visible and window discovery will retry.'
    );
    return;
  }
...
async function promoteLoadingHome(overlay: BrowserWindow, gameId: string): Promise<void> {
  const generation = gameplayQuickOverlayTransitionGeneration;
  const stillCurrent = () => gameplayQuickOverlayDesiredOpen &&
    gameplayLaunchCoverGameId === gameId && launcher.activeState.game?.id === gameId &&
    generation === gameplayQuickOverlayTransitionGeneration && !overlay.isDestroyed();
  if (!stillCurrent() || launcher.isLaunchInProgress || launcher.isManagingGames) return;

  // Keep the loading window opaque until both the real game visual and native
  // stacking are ready. The Home menu stays usable throughout discovery.
  await prepareGameplayQuickOverlayRenderer(true, true, true, true);
  if (!stillCurrent()) return;
  if (!gameplayQuickOverlayRendererReady ||
      !['direct', 'live'].includes(gameplayQuickOverlayPreparedBackdropKind ?? '') ||
      !gameplayQuickOverlayPreparedWindowHandle) return;
  if (!await stageGameplayQuickOverlay(overlay, gameId, true) || !stillCurrent()) return;

  gameplayLaunchCoverGameId = null;
  if (gameplayLaunchCoverFocusTimer) clearInterval(gameplayLaunchCoverFocusTimer);
  gameplayLaunchCoverFocusTimer = null;
  overlay.setBackgroundColor('#00000000');
  await logLine('info', `Loading Home upgraded to ${gameplayQuickOverlayPreparedBackdropKind} gameplay for ${launcher.activeState.game?.title}.`);
}
...
function protectLoadingHome(overlay: BrowserWindow, gameId: string): void {
  const keepHomeVisible = () => {
    ...
    if (!loadingHomePromotion && Date.now() >= loadingHomeNextProbeAt && !launcher.isLaunchInProgress) {
      const operation = promoteLoadingHome(overlay, gameId);
      loadingHomePromotion = operation;
      void operation.catch(error => logLine('warn', `Loading Home live upgrade will retry: ${String(error)}`))
        .finally(() => {
          if (loadingHomePromotion === operation) loadingHomePromotion = null;
          loadingHomeNextProbeAt = Date.now() + 1000;
        });
    }
  };
  keepHomeVisible();
...
async function performGameplayQuickOverlayTransition(
  ...
  if (transitionGeneration !== gameplayQuickOverlayTransitionGeneration) {
    return { ok: true };
  }
  ...
  const overlayWasVisible = Boolean(getLiveGameplayQuickOverlayWindow()?.isVisible());
  if (
    !overlayWasVisible &&
    (launcher.activeState.status === 'running' || launcher.activeState.status === 'launching')
  ) {
    // A second Home press while Store-window discovery is pending cancels
    // only the requested overlay. It must not cancel or restart the launch.
  ...
  try {
    ...
    if (
      ...
      // The next serialized transition owns the final foreground state.
      // Never start a competing show while this resume is still unwinding.
      return result;
    }
    ...
    if (result.ok) {
      // The tracked game is now visible and foreground. Hiding the overlay
      // only after that confirmation prevents desktop or app flashes.
...
function transitionGameplayQuickOverlay(
  ...
  const operation = gameplayQuickOverlayTransitionQueue
    .catch(() => undefined)
    .then(async (): Promise<GameControlResult> => {
      if (transitionGeneration !== gameplayQuickOverlayTransitionGeneration) {
        await logLine(
          'info',
          `Gameplay Home transition ${transitionGeneration} was superseded before touching native focus.`
        );
        return { ok: true };
      }
      return performGameplayQuickOverlayTransition(
        shouldOpen,
        reason,
  ...
  gameplayQuickOverlayTransitionQueue = operation.then(
    () => undefined,
    () => undefined
  );
  return operation;
...
function handleShellHomeRequest(reason: ShellHomeReason): void {
  ...
  if (reason !== 'emergency-close' && requestedAt - lastShellHomeRequestAt < 80) {
