...
try {
  ...
  const quickPanel = await evaluate("(() => { const panel = document.querySelector('#quick-settings-panel'); const rows = [...panel.querySelectorAll('.quick-settings-control')]; return { rowCount: rows.length, audioFirst: rows[0]?.classList.contains('quick-settings-audio'), brightnessSecond: rows[1]?.classList.contains('quick-settings-brightness'), hasOpenSettings: /open settings/i.test(panel.textContent), panelBottom: Math.round(panel.getBoundingClientRect().bottom), gearTop: Math.round(document.querySelector('button[aria-label=\"Quick Settings\"]').getBoundingClientRect().top) }; })()");
  if (quickPanel.rowCount !== 2 || !quickPanel.audioFirst || !quickPanel.brightnessSecond || quickPanel.hasOpenSettings || quickPanel.panelBottom > quickPanel.gearTop) throw new Error(`Quick Settings structure or attachment is incorrect: ${JSON.stringify(quickPanel)}`);
  ...
  await waitFor("window.nxgs.getAudioStatus().then((audio) => Number(document.querySelector('#quick-settings-audio').value) === audio.masterVolume)", 'Quick Settings did not show the current master volume.');
  await waitFor("window.nxgs.getDisplayStatus().then((display) => display.brightness.supported ? Number(document.querySelector('#quick-settings-brightness').value) === display.brightness.level : document.querySelector('.quick-settings-brightness small')?.textContent.trim().length > 0)", 'Quick Settings did not show the actual brightness or a clear unsupported state.');
